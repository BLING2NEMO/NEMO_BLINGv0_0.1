...
CONTAINS
   ...
   SUBROUTINE trc_ext_bling (kt) 
      ...
      REAL(wp) :: sss, sst, o2fact, rho0, kfact
      ...
      DO jj=1,jpj
        DO ji=1,jpi
          ...
          csurf_o2=trn(ji,jj,1,jpOxy_bling)*rho0*sch_no_term*kfact

          ! Flux of oxygen (mol/m2/s)
          o2flx   =alpha_o2-csurf_o2

          trn(ji,jj,1,jpOxy_bling)=trn(ji,jj,1,jpOxy_bling)+o2flx/rho0*zrfact
      ...
      DO jj = 1, jpj
         DO ji = 1, jpi
            zrfact  = rfact / (epsln+fse3t(ji,jj,1))
            trn(ji,jj,1,jpFed_bling) =  trn(ji,jj,1,jpFed_bling) &
                                      + dust_bling(ji,jj)/1.e3*zrfact
            dum1(ji,jj,10)=dust_bling(ji,jj)
         ENDDO
      ENDDO
...
dum1(ji,jj,11)=ikb
            ...
            sumbfpo4(ji,jj)=sumbfpo4(ji,jj)+bfpo4(ji,jj)
            sumbffed(ji,jj)=sumbffed(ji,jj)+bffed(ji,jj)
            sumbfoxy(ji,jj)=sumbfoxy(ji,jj)+bfoxy(ji,jj)

            dum1(ji,jj,4)=sumbfpo4(ji,jj)*1.d6
            dum1(ji,jj,5)=sumbffed(ji,jj)*1.d9
            dum1(ji,jj,6)=sumbfoxy(ji,jj)*1.d6
