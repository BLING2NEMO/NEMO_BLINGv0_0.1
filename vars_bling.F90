MODULE vars_bling
   ...
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:,:) :: dum1, dum2
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:)   :: fpop_b, fpofe_b
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:)   :: sumbfpo4, sumbffed, sumbfoxy
   ...
   !! Total mass lost to sediments
   REAL(wp) :: sumblpo4, sumblfed, sumbloxy
...
CONTAINS
   ...
   INTEGER FUNCTION bling_alloc()
     ...
     ALLOCATE(      dum1(jpi,jpj,jpk), dum2(jpi,jpj,jpk)               , STAT=ierr(2) )
     ! bottom fluxes
     ALLOCATE(  sumbfpo4(jpi,jpj), sumbffed(jpi,jpj), sumbfoxy(jpi,jpj) &
               ,  fpop_b(jpi,jpj),  fpofe_b(jpi,jpj)                   , STAT=ierr(3) )
